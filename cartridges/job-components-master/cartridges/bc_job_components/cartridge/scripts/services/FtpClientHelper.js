 ...
 * If true, the underlying FTP connection will be disconnected after the service call.
 * If false then it will remain open. The default value is true.
 *
 * @param {Boolean} autoDisconnectFlag flag for controlling the auto disconnect of the service
 */
FtpClientHelper.prototype.setAutoDisconnect = function (autoDisconnectFlag) {
    this.service.setAutoDisconnect(autoDisconnectFlag);
};

/**
 * Disconnects the Service Client
 */
FtpClientHelper.prototype.disconnectClient = function () {
    const serviceClient = this.service.getClient();
