...
var run = function run() {
    ...
    var connectionKeepAlive = args.ConnectionKeepAliveEnabled;
    ...
    // Set the auto disconnect flag
    serviceClient.setAutoDisconnect(!connectionKeepAlive);
    ...
    var fileList;
    if (recursive) {
        fileList = FileHelper.getFileListRecursive(sourceDirStr, filePattern, sourceFolder, targetFolder, recursive);
    } else {
        fileList = FileHelper.getFiles(sourceDirStr, filePattern);
    }
    if (fileList.length === 0) {

        // Manually disconnect the client in case the auto disconnect is set to false
        if (connectionKeepAlive) {
            serviceClient.disconnectClient();
        }
