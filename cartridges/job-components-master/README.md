...
#### custom.ExtractAllPageDesignerPages

 - Exports all page designer pages, from a given source library file
 - content to export is identified by prefix "page" or "component" in the attribute "type"
 - also exports all folders, which are in the source library file
 - file path of source file is checked for leading and ending slash "/"
 - file extension of source file name is checked (if not available, ".xml" is added)
 - file path of output file is checked for leading and ending slash "/"
 - file extension of output file name is checked (if not available, ".xml" is added)

#### custom.ExtractSpecificPageDesignerPages

- Exports specific page designer pages based on given IDs from job parameter "PDPageIDs"
- the job step consists of 3 steps: find all requested page designer pages and check them for linked content, loop through all found pages and find every sub-page and write all pages and sub-pages to the output xml file 
- content to export is identified by prefix "page" or "component" in the attribute "type"
- also exports all folders, which are in the source library file
- file path of source file is checked for leading and ending slash "/"
- file extension of source file name is checked (if not available, ".xml" is added)
...
#### custom.BackupCategoryRules

- Exports category rules in a date specific folder with pattern <categoryId>.json
- requires service and credentials for account manager and scapi - if you don't have that already you can find a sample site importe [here](https://github.com/SalesforceCommerceCloud/job-components/tree/master/site_template/backup-category-rules)
...
### [0.0.3](SalesforceCommerceCloud/job-components/releases/tag/0.0.3) 2023-02-08

### Fixed
 - steptypes.json syntax so it validates for SFCC v22.1.2.24 and later
 - Bug related to file paths with separators
...
#### Added
 ...
 - Placeholders for export queries
